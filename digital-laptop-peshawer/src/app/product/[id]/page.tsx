...
import { SHOP, buildWhatsAppOrderUrl } from "@/lib/constants";
...
function SpecRow({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value?: string | null }) {
  ...
  return (
    <div className="flex items-start gap-4 rounded-2xl border border-white/5 bg-white/[0.02] px-5 py-4">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm font-medium leading-relaxed text-zinc-100">{value}</p>
      </div>
    </div>
  );
}
...
export default async function ProductPage({ params }: { params: Params }) {
  ...
  const product = await getProductById(productId).catch(() => undefined);
  if (!product) notFound();
  ...
  const waMessage = buildWhatsAppOrderUrl(
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 lg:px-8">
          ...
          <Reveal delay={0.08}>
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <Badge><ShieldCheck className="h-3 w-3" /> {product.warranty}</Badge>
                <Badge><Banknote className="h-3 w-3" /> Cash on Delivery</Badge>
              </div>
              ...
              <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.03] p-5 backdrop-blur-xl">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-white/5 pt-4 text-xs text-zinc-500">
                  <span className="inline-flex items-center gap-1.5"><Truck className="h-3.5 w-3.5 text-cyan-300/70" /> Same-day delivery in Peshawar</span>
                  <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5 text-cyan-300/70" /> DLS lab tested</span>
                </div>
              </div>
              ...
              >
                <MessageCircle className="h-4.5 w-4.5" /> Ask about this laptop on WhatsApp
              </a>
            </div>
          </Reveal>
        </div>
        ...
        <Reveal className="mt-20">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <SpecRow icon={MemoryStick} label="Memory (RAM)" value={product.ram} />
            ...
            <SpecRow icon={BatteryCharging} label="Battery" value={product.battery} />
            <SpecRow icon={ShieldCheck} label="Warranty" value={product.warranty} />
            <SpecRow icon={Truck} label="Delivery" value="Cash on Delivery — Peshawar" />
          </div>
        </Reveal>
        ...
        {related.length > 0 && (
          <Reveal className="mt-24">
            <div className="flex items-end justify-between gap-6">
              ...
              <Link href="/shop" className="text-sm text-zinc-400 transition-colors hover:text-white">
                View all →
              </Link>
            </div>
