...
export default async function OrderSuccessPage({ params }: { params: Params }) {
  ...
  const [details, config] = await Promise.all([
    Number.isFinite(orderId)
      ? getOrderWithDetails(orderId).catch(() => undefined)
  ...
  const waUrl = `${config.whatsapp}?text=${encodeURIComponent(lines.join("\n"))}`;
      ...
      <div className="relative mx-auto max-w-3xl px-4 pb-24 pt-32 sm:px-6">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-4 max-w-md text-zinc-400">
            Thank you, <span className="text-white">{customer.fullName}</span>.
            Your order has been received. Confirm it on WhatsApp and we&apos;ll
          ...
          <p className="mt-4 max-w-md text-sm text-zinc-400">
            Save this order number — you can track your order status anytime on
            the{" "}
            <Link
              href="/track"
            ...
            page using {number} or your phone number.
          </p>
        </div>
