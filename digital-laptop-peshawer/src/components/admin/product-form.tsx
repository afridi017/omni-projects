  ...
  async function handleFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files).slice(0, 8)) {
      ...
      try {
        ...
        fd.append("file", file);
        fd.append("folder", "digital-laptop-products");
        ...
        setDraft((d) => ({
          ...d,
          images: [...d.images, data.url].slice(0, 8),
        }));
      } catch (err) {
        setError(
  ...
  return (
    <Card className="mx-auto max-w-4xl p-6 sm:p-8">
      ...
      <p className="mt-1 text-sm text-zinc-500">
        Fields marked * are required. Images are uploaded to Cloudinary or added
        by URL.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <Field label="Processor *">
            <Input
              ...
              placeholder="e.g. Intel Core i7-13700H (13th Gen)"
            />
          </Field>
          <Field label="RAM *">
            <Input
          ...
          <Field label="Display *">
            <Input
              ...
              placeholder='e.g. 15.6" FHD 120Hz IPS'
            />
          </Field>
          <Field label="Battery">
            <Input
              ...
              placeholder="e.g. 4–5 hrs backup, 86Wh"
            />
          </Field>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div
            ...
            className="cursor-pointer rounded-2xl border border-dashed border-white/15 bg-white/[0.02] px-6 py-8 text-center transition-colors hover:border-cyan-400/40 hover:bg-cyan-400/[0.03]"
          >
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Uploaded securely to Cloudinary — PNG / JPG / WebP (max 8 MB each)
            </p>
            {uploading > 0 && (
              <p className="mt-2 inline-flex items-center gap-1.5 text-xs text-cyan-300">
                <Loader2 className="h-3.5 w-3.5 animate-spin" /> Uploading{" "}
                {uploading} image{uploading > 1 ? "s" : ""}...
              </p>
            )}
          </div>
          <input
          ...
          {draft.images.length > 0 && (
            <div className="flex flex-wrap gap-3 pt-1">
              {draft.images.map((url, i) => (
                <div
                  key={url + i}
                  className="group relative h-20 w-28 overflow-hidden rounded-xl border border-white/10 bg-[#0a0c12]"
                >
        ...
        >
          <span>
            ...
            <span className="mt-0.5 block text-xs text-zinc-500">
              Featured laptops appear in the homepage &quot;Featured
              Laptops&quot; section.
            </span>
          </span>
          <span
